...
function createTransactionSuccess(body,req,res){
  var values = [req.sanitize(body.creditAccount),req.sanitize(body.debitAccount),req.sanitize(body.creditAadharNo),req.sanitize(body.debitAadharNo),req.sanitize(body.amount),4];
  connection.query('insert into transaction(CreditAccount, DebitAccount, CreditAadharNo, DebitAadharNo, Amount, Status) values(?,?,?,?,?,?)',values, function(err, rows, fields) {
    if (!err){
      ...
      if(req.sanitize(body.amount) < 20000){
          addRequest("transactionreqre",rows.insertId,req,res);
      }else if(req.sanitize(body.amount) >= 20000 && req.sanitize(body.amount) < 50000){
            addRequest("transactionreqsa",rows.insertId,req,res);
      }else if(req.sanitize(body.amount)>= 50000){
            addRequest("transactionreqsm",rows.insertId,req,res);
      }
  }
    else{
